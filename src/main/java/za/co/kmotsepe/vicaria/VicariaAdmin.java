...
/**
 * @author Benjamin Kohl <bkohl@users.sf.net>
 * @author Artem Melnik <mandara@mac.com>
 * @author Kingsley Motsepe
 * @since %G%
 * @version %I%
 */
public class VicariaAdmin {
    ...
    // TODO this should come from a MANIFEST file [jcabi-manifests]
     ...
     * @param server
     */
    public VicariaAdmin(String filename, VicariaServer server) {
        VicariaAdmin.server = server;
        this.filename = filename;
    }

    public void WebAdmin() throws IOException {
        // String filename = filename;
        ParseRequest(this.filename);
                    ...
                    switch (command) {
                        ...
                        case "savefilter":
                            ...
                            if (filterurl != null && action != null) {
                                VicariaURLMatch a = new VicariaURLMatch(filterurl, false/* set to real */,
                                        Integer.parseInt(action), ""/* set to real! */); // add new url match
                        ...
                        case "global": {
                            ...
                            VicariaServer.block_urls = (a != null);
    ...
    public void ParseRequest(String filename) {
        ...
        if (ixQmark != -1) {
            String request = VicariaUtils.urlDecoder(filename.substring(ixQmark + 1, filenamelen));
            if (server.debug) {
            ...
            if (server.debug) {
                LOGGER.debug("\trequesturl: " + filename.substring(0, ixQmark));
            }
            ...
            if (server.debug) {
                LOGGER.debug("\trequesturl: " + filename);
            }
        }
    }
    ...
    /**
     * generate String representation of the HTML form
     *
     * @return
     */
    public String HTMLAdmin() {
        ...
        if (server.config_auth == 0) {
            ...
            msg = msg
                    ...
                    + chk_option("server.webconfig <br><i>Enables the web-based configuration module</I>", "webconfig",
                            server.webconfig)
                    ...
                    + chk_option("server.filter.url", "block_urls", VicariaServer.block_urls)
                    ...
                    // +txt_field ("server.webconfig.username", "config_username",
                    // server.config_user)
                    // +txt_field ("server.webconfig.password", "config_password",
                    // server.config_password,true)
                    + "<tr>"
                    ...
                    + "</form>"
                    + HTMLFiltersList()
                    + "<a href=\"/" + server.WEB_CONFIG_FILE + "?command=logout&sid=" + server.config_session_id
                    + "\">Logout</a>&nbsp;<a href=\"/" + server.WEB_CONFIG_FILE + "?command=shutdown&sid="
                    + server.config_session_id + "\">Shutdown Server</a>";
        }

        msg = msg
                // + "<HR size=\"4\">\r"
                + "<p class=\"i25\"><A HREF=\"http://jhttp2.sourceforge.net/\">jHTTPp2</A> HTTP Proxy, Version "
                + VicariaServer.getServerVersion() + "\r"
    ...
    public static String txt_field(String a1, String o, String o2, String w) {
        return "<tr>\r"
                ...
                + "<td width=\"449\" height=\"6\">" + o + " <input type=\"text\" name=\"" + o2
                + "\" size=\"20\" value=\"" + w + "\"></td>\r"
                + "</tr>\r";
    }
    ...
    public static String txt_field(String a1, String o, String o2, String w, boolean b) {
        return "<tr>\r"
                + "<td width=\"52\" height=\"6\">" + a1 + "</td>\r"
                + "<td width=\"449\" height=\"6\">" + o + " <input type=\"password\" name=\"" + o2
                + "\" size=\"20\" value=\"" + w + "\"></td>\r"
                + "</tr>\r";
    }
    ...
    public static String HTMLActionPopup() {
        ...
        Enumeration an = Collections.enumeration(server.getURLActions());
        int i = 0;
    ...
    public static String HTMLFiltersList() {
        ...
        out.append("<form method=\"POST\" action=\"/" + server.WEB_CONFIG_FILE
                + "\">\r<input type=\"hidden\" name=\"command\" value=\"savefilter\"/><input type=\"hidden\" name=\"sid\" value=\"")
                .append(server.config_session_id)
                .append("\"/><b>URL Filter Settings</b><br>New/Update Filter: <input type=\"text\" name=\"filter\" size=\"20\" value=\"\">Action: <input type=\"text\" name=\"action\" size=\"10\" value=\"1\"> <input type=\"submit\" value=\"Save\" /><br><p style=\"font-size: 10px;\">");
        while (an.hasMoreElements()) {
            VicariaURLMatch a = (VicariaURLMatch) an.nextElement();
            String aname = a.getMatch();

            out.append(a.getMatch()).append(" [").append(Integer.toString(a.getActionIndex()))
                    .append("] <a href=\"/" + server.WEB_CONFIG_FILE + "?command=deletefilter&filterid=").append(aname)
                    .append("&sid=").append(server.config_session_id).append("\">Delete Filter</a><br>");
        }
        out.append("</p></form>");

 ...
 * One HTTP connection
 *
 * @file VicariaHTTPSession.java
 * @author Benjamin Kohl
 * @author Kingsley Motsepe
 * @since %G%
 * @version %I%
 */
public class VicariaHTTPSession extends Thread {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(VicariaHTTPSession.class);
    ...
    private VicariaClientInputStream in;
    ...
    private VicariaServerInputStream HTTP_in;

    public VicariaHTTPSession(VicariaServer server, Socket client) {
        try {
            in = new VicariaClientInputStream(server, this, client.getInputStream());// ,true);
            out = new BufferedOutputStream(client.getOutputStream());
            VicariaHTTPSession.server = server;
            this.client = client;
        } catch (IOException e_io) {
            try {
                client.close();
            } catch (IOException e_io2) {
                LOGGER.error(e_io2.getMessage());
            }
            LOGGER.error("Error while creating IO-Streams: " + e_io.getMessage());
        }
        startSelf();
    }
    ...
    public void run() {
        ...
        LOGGER.debug("beging http session");
        // }
        ...
        try {
            ...
            // if (server.debug) {
            LOGGER.error(e_handleRequest.toString());
            // }
        } catch (Exception e) {
            // e.printStackTrace();
        ...
        try {
            ...
            LOGGER.error(e_run.getMessage());
        }
        server.decreaseNumConnections();
        if (server.debug) {
            LOGGER.info("end http session");
        }
    }
    ...
    public void sendErrorMSG(int a, String info) throws IOException {
        ...
        try {
            ...
            LOGGER.error(e_unknown_host.getMessage());
        }

        stringBuilder.append(
                "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Transitional//EN\"><html>\r<!-- jHTTPp2 error message --><HEAD>\r<TITLE>")
                .append(statuscode)
                .append("</TITLE>\r<link rel=\"stylesheet\" type=\"text/css\" href=\"http://")
                .append(localhost)
                .append("/style.css\"></HEAD>\r<BODY BGCOLOR=\"#FFFFFF\" TEXT=\"#000000\" LINK=\"#000080\" VLINK=\"#000080\" ALINK=\"#000080\">\r<h2 class=\"headline\">HTTP ")
                .append(statuscode)
                .append(" </h2>\r<HR size=\"4\">\r<p class=\"i30\">Your request for the following URL failed:</p><p class=\"tiagtext\"><a href=\"")
                .append(in.getFullURL())
                .append("\">")
                .append(in.getFullURL())
                .append("</A> </p>\r<P class=\"i25\">Reason: ")
                .append(info)
                .append("</P><HR size=\"4\">\r<p class=\"i25\"><A HREF=\"http://jhttp2.sourceforge.net/\">jHTTPp2</A> HTTP Proxy, Version ")
                .append(VicariaServer.getServerVersion())
                .append(" at ")
                .append(localhost)
                .append("<br>Copyright &copy; 2001-2003 <A HREF=\"mailto:bkohl@users.sourceforge.net\">Benjamin Kohl</A></p>\r<p class=\"i25\"><A HREF=\"http://")
                .append(localhost)
                .append("/\">jHTTPp2 local website</A> <A HREF=\"http://")
                .append(localhost)
                .append("/" + server.WEB_CONFIG_FILE
                        + "\">Configuration</A></p></BODY></HTML>");
        sendLine("Content-Length", String.valueOf(stringBuilder.length()));
        sendLine("Content-Type", "text/html; charset=iso-8859-1");
        endHeader();
        write(out, stringBuilder.toString());
        out.flush();
    }
                ...
                if (in.getStatusCode() != SC_CONNECTING_TO_HOST) {
                    switch (in.getStatusCode()) {
                        ...
                        case SC_CLIENT_ERROR:
                            sendErrorMSG(400, "Your client sent a request that this proxy could not understand. ("
                        ...
                        case SC_NOT_SUPPORTED:
                            sendErrorMSG(501, "Your client used a HTTP method that this proxy doesn't support: ("
                                    + in.getErrorDescription() + ")");
                            break;
                        case SC_URL_BLOCKED:
                            sendErrorMSG(403,
                    ...
                    // if (server.debug)server.writeLog("Connect: " + remote_host + ":" +
                    // remote_port);
                    try {
                        connect(remote_host, remote_port);
                    } catch (IOException e_connect) {
                        // if (server.debug) {
                        LOGGER.error(e_connect.getMessage());
                        // server.writeLog(e_connect.toString());
                        // }
                        sendErrorMSG(502, "Error while creating a TCP connecting to [" + remote_host.getHostName() + ":"
                                + remote_port + "] <BR>The proxy server cannot connect to the given address or port ["
                                + e_connect.toString() + "]");
                        break;
                    } catch (Exception e) {
                    ...
                    if (!in.isTunnel() || (in.isTunnel() && server.use_proxy)) { // no SSL-Tunnel or SSL-Tunnel with
                                                                                 // another remote proxy: simply forward
                                                                                 // the request
                    ...
                    remote_in = new VicariaRead(server, this, HTTP_in, out); // reads data from the remote server
                    server.addBytesWritten(numread);
                }
            }
            while (true) { // reads data from the client
                numread = in.read(b);
                // if (server.debug)server.writeLog("Jhttpp2HTTPSession: " + numread + " Bytes
                // read.");
                if (numread != -1) {
                    HTTP_out.write(b, 0, numread);
                    HTTP_out.flush();
    ...
    public void connect(InetAddress host, int port) throws IOException {
        ...
        HTTP_in = new VicariaServerInputStream(server, this, HTTP_Socket.getInputStream(), false);
    ...
    public void file_handler() throws IOException {
        ...
        endHeader();
        try (BufferedInputStream file_in = new BufferedInputStream(new FileInputStream(file))) {
    ...
    public void admin_handler(byte[] b) throws IOException {
        ...
        String adminpage = "";
        try {
            admin = new VicariaAdmin(filename, server);
            admin.WebAdmin();
        ...
        if (adminlen < 1) {
            sendErrorMSG(500, "Error Message from the Web-Admin modul: " + VicariaAdmin.error_msg);
        } else {
